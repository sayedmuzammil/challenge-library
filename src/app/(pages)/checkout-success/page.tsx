...
export default function BorrowSuccessPage() {
  ...
  // Accept either ?due=YYYY-MM-DD or ?due=timestamp
  const dueRaw = params.get('due') ?? '';
  const due = dueRaw
    ? new Date(isNaN(Number(dueRaw)) ? `${dueRaw}T00:00:00` : Number(dueRaw))
    : new Date(Date.now() + 1000 * 60 * 60 * 24 * 7); // fallback: +7 days

  return (
    <main className="mx-auto flex min-h-[70vh] max-w-2xl flex-col items-center justify-center px-6 text-center text-slate-800">
      {/* Circle check icon */}
      <div className="relative mb-6 h-24 w-24">
        <div className="absolute inset-0 rounded-full bg-blue-600"></div>
        <div className="absolute inset-0 rounded-full ring-8 ring-blue-100"></div>
        <svg
          viewBox="0 0 24 24"
